...
BOOL MainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	if (m_wndSplitter.CreateStatic(this, 1, 2, WS_CHILD | WS_VISIBLE) == FALSE) {
		TRACE(_T("Failed when create static"));
		return FALSE;
	}
	if (m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(FormView), CSize(200, 500), pContext) == FALSE) {
		TRACE(_T("Failed when create view"));
		return FALSE;
	}
	if (m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(OpenglView), CSize(500, 500), pContext) == FALSE) {
		TRACE(_T("Failed when create View"));
		return FALSE;
	}

	//form_view_ = reinterpret_cast<FormView*>(m_wndSplitter.GetPane(0, 0));
	app_view_ = reinterpret_cast<OpenglView*>(m_wndSplitter.GetPane(0, 1));
	return TRUE;
}
